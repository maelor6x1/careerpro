...
const Career = (() => {
  ...
  function buildCompetitions() {
    ...
    if (leagueMeta.format === 'zones') {
      ...
      state.competitions = {
        phase: estadual ? 'estadual' : 'liga', estadual, liga, copa: null,
  ...
  function concludeRound() {
    ...
    if (state.week % 5 === 0) Transfers.generateSponsorOffers(state.player).forEach(s => state.inbox.push({ type: 'sponsor_offer', data: s, id: uid() }));
