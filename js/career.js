...
const Career = (() => {
  ...
  function buildCompetitions() {
    ...
    const comps = DB.COMPETITIONS[league] || [DB.LEAGUES[league].name];

    let estadual = null;
    if (league === 'BRA' && state.club.state) {
      const stateClubIds = DB.clubsByState('BRA', state.club.state).map(c => c.id);
      if (stateClubIds.length >= 3) {
        estadual = League.buildLeaguePhase(DB.ESTADUAIS[state.club.state] || 'Campeonato Estadual', stateClubIds);
      }
    }

    const ligaName = DB.LEAGUES[league].name;
    const liga = League.buildLeaguePhase(ligaName, leagueClubIds);

    const cupName = comps.find(c => /copa|cup|taça|pokal|coupe/i.test(c)) || comps[1] || comps[0];
    const cupSeeds = leagueClubIds.slice().sort((a, b) => clubRep(b) - clubRep(a)).slice(0, Math.min(8, leagueClubIds.length));
    const copa = League.buildCupPhase(cupName, shuffleArr(cupSeeds));

    state.competitions = { phase: estadual ? 'estadual' : 'liga', estadual, liga, copa };
  }
  ...
  function advancePhase() {
    const order = ['estadual', 'liga', 'copa', 'offseason'];
  ...
  // -----------------------------------------------------------------------
  // INTERACTIVE MATCH — begin / step / conclude
  // -----------------------------------------------------------------------
  function beginPlayerMatch() {
    const pending = state._pendingRound;
    const beats = Events.planMatch(state.player);
    state.currentMatch = {
      beats, beatIndex: 0,
      ctx: { stats: { goals: 0, assists: 0, yellow: 0, red: 0 }, injury: null },
      log: [],
      opponentId: pending.roundData.playerFixture.home === state.club.id ? pending.roundData.playerFixture.away : pending.roundData.playerFixture.home,
      isHome: pending.roundData.playerFixture.home === state.club.id,
    };
    save();
    return nextBeatPacket();
  }

  function nextBeatPacket() {
    const m = state.currentMatch;
    if (!m) return { matchOver: true };
    if (m.beatIndex >= m.beats.length || m.ctx.stats.red > 0 || m.ctx.injury) return { matchOver: true };
    const beat = m.beats[m.beatIndex];
    return { matchOver: false, beat };
  }

  // Called by the UI: pass optionKey for decision beats, omit for auto beats.
  function stepMatch(optionKey) {
    const m = state.currentMatch;
    const beat = m.beats[m.beatIndex];
    const result = beat.type === 'decision'
      ? Events.resolveDecision(beat, optionKey, state.player, m.ctx)
      : Events.resolveAuto(beat, state.player, m.ctx);
    if (m.ctx.injury) m.ctx.injury = m.ctx.injury; // already set on ctx by resolver
    m.log.push(result.text);
    m.lastMood = (m.lastMood || 0) + result.mood;
    m.beatIndex += 1;
    save();
    return { result, next: nextBeatPacket() };
  }
  ...
  function concludeRound() {
    ...
    if (state.currentMatch) {
      ...
      const baseline = League.simulateCpuMatch(state.club, opponent);
      const teammateGoals = Math.round(baseline.homeGoals * 0.55);
      const playerTeamGoals = teammateGoals + m.ctx.stats.goals;
      const opponentTeamGoals = baseline.awayGoals + (m.ctx.stats.concededOwnGoal ? 1 : 0);
    ...
    if (state.week % 6 === 0) Transfers.generateSponsorOffers(state.player).forEach(s => state.inbox.push({ type: 'sponsor_offer', data: s, id: uid() }));
    const callUp = Transfers.checkCallUp(state.player, state);
    if (callUp) state.inbox.push({ type: 'call_up', data: callUp, id: uid() });
    if (summary.played && summary.rating >= 7.8) state.inbox.push({ type: 'interview', id: uid(), data: {} });
  ...
  return {
    startNewCareer, chooseClub, coachDecision, prepareRound, beginPlayerMatch, stepMatch, concludeRound,
    train, endSeason, acceptTransfer, rejectTransfer, acceptSponsor, rejectSponsor,
    acceptCallUp, declineCallUp, answerInterview, getStandings, getBracket,
