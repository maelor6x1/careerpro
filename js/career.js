...
const Career = (() => {
  ...
  function buildContinentalPhase(league) {
    ...
    const tier = qualificationTierFor(state.club.id, league);
    if (!tier) return null;
    ...
    const entrants = poolIds.filter(id => qualificationTierFor(id, leagueOf(id)) === tier);
    if (!entrants.includes(state.club.id)) entrants.push(state.club.id);

    if (confInfo === DB.CONTINENTAL.CONMEBOL) {
      // Libertadores/Sul-Americana: real group-stage format, groups of 4.
      const size = Math.max(8, entrants.length - (entrants.length % 4 || 4));
      const seeds = shuffleArr(entrants).slice(0, Math.min(32, Math.max(8, entrants.length)));
      if (!seeds.includes(state.club.id)) seeds[0] = state.club.id;
      return League.buildGroupStagePhase(compName, seeds, 4);
    }
    // Champions/Europa League: Swiss-style league phase.
    const seeds = shuffleArr(entrants).slice(0, Math.min(36, Math.max(8, entrants.length)));
    if (!seeds.includes(state.club.id)) seeds[0] = state.club.id;
  ...
  function concludeRound() {
    ...
    if (state.currentMatch) {
      ...
      if (pending.isCup && playerTeamGoals === opponentTeamGoals) {
        ...
        summary.extraTimeLog = [`⏱️ Fim dos 90 minutos empatado! Vamos para a prorrogação.`];
        if (playerTeamGoals !== opponentTeamGoals) summary.extraTimeLog.push(`⚽ Gol decidido na prorrogação!`);
      }
      if (pending.isCup && playerTeamGoals === opponentTeamGoals) {
        penalties = true;
        const gkBonus = state.player.position === 'GOL' ? (state.player.attributes.reflexo - 60) * 0.003 : 0;
        const win = Math.random() < (0.5 + gkBonus);
        summary.penaltiesLog = ['🥅 Nos pênaltis!'];
        if (win) playerTeamGoals += 1; else opponentTeamGoals += 1; // nominal +1 just to break the tie for scoreline purposes
      }
      ...
      state.matchLog.unshift({ text: `${summary.homeTeam} ${scoreLabel} ${summary.awayTeam} — ${m.log.join(' ')}`, rating: m.rating, week: state.week });
  ...
  function getAwardRaces() {
    ...
    // Sample across every league in the game, not just the player's own —
    // a handful of the biggest clubs from each, so the leaderboard actually
    // reflects a worldwide race instead of one domestic league's table.
    const allLeagueCodes = Object.keys(DB.LEAGUES);
    const candidateClubs = [];
    allLeagueCodes.forEach(code => {
      const top = DB.clubsByLeague(code).slice().sort((a, b) => b.reputation - a.reputation).slice(0, 4);
      candidateClubs.push(...top);
    });

    const attackPool = []; // { pl, club } — forwards/wingers/attacking mids, for goals
    const generalPool = []; // broader mix, for Ballon d'Or
    const keeperPool = []; // goalkeepers only

    candidateClubs.forEach(c => {
      const squad = c.id === state.club.id ? state.squad : PlayerModel.generateSquad(c);
      const attackers = squad.filter(pl => ['ATA', 'PE', 'PD', 'MEIA'].includes(pl.position)).sort((a, b) => b.overall - a.overall).slice(0, 2);
      const keeper = squad.filter(pl => pl.position === 'GOL').sort((a, b) => b.overall - a.overall)[0];
      const bestOverall = squad.slice().sort((a, b) => b.overall - a.overall).slice(0, 2);
      attackers.forEach(pl => attackPool.push({ pl, club: c }));
      bestOverall.forEach(pl => generalPool.push({ pl, club: c }));
    ...
    // de-dupe by name+club (same player can appear via both pools)
  ...
  function signWithNewClub(club) {
    const contractOffer = Transfers.makeContract(club, state.player, { years: 3, wageMultiplier: 1 + Math.random() * 0.4, startYear: state.year });
    state.club = club; state.league = club.league;
    state.squad = PlayerModel.generateSquad(club);
    state.player.contract = contractOffer;
    buildCompetitions();
  ...
  return {
    ...
    train, endSeason, getRenewalOffer, renewContract, getInterestedClubs, signWithNewClub,
    acceptTransfer, rejectTransfer, acceptSponsor, rejectSponsor,
    acceptCallUp, declineCallUp, answerInterview, getStandings, getBracket, getAwardRaces, getUpcomingFixtures,
