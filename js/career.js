...
const Career = (() => {
  ...
  function prepareNationalTournamentRound() {
    ...
    if (nt.stage === 'group') {
      if (nt.roundIndex >= nt.rounds.length) {
        ...
        if (myRank < 2) { nt.stage = 'knockout'; nt.knockoutRound = 'semi'; return prepareNationalTournamentRound(); }
  ...
  function concludeNationalTournamentMatch() {
    ...
    if (m.stage === 'group') {
      ...
      if (m.score.for > m.score.against) { nt.knockoutRound = 'final'; summary.advanced = true; }
      else { nt.finished = true; nt.eliminated = true; summary.eliminated = true; pushNews('Eliminado na semifinal', `${nt.nationality} cai na semifinal da ${nt.competition}.`); }
    } else if (m.stage === 'final') {
      nt.finished = true;
      if (m.score.for > m.score.against) {
        nt.champion = true; summary.championWon = true;
        state.player.trophies.push(`${nt.competition} — ${state.year}`);
        pushNews('🏆 CAMPEÃO DO MUNDO!', `${nt.nationality} conquista a ${nt.competition} com ${p.name} em campo!`);
      } else {
    ...
    pushNews('Seleção', `${p.name} atua por ${m.myTeam.shortName} (${nt.competition}): ${homeTeam} ${homeGoals}x${awayGoals} ${awayTeam}.`);
    state.currentMatch = null;
