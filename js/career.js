  ...
  function concludeRound() {
    ...
    if (state.currentMatch) {
      ...
      if (!pending.byeThisRound && pending.roundData.playerFixture) {
        ...
        const { homeGoals, awayGoals } = League.simulateCpuMatch(homeClub, awayClub);
        ...
        if (pending.isCup) {
          const winner = homeGoals === awayGoals ? (Math.random() < 0.5 ? fixture.home : fixture.away) : (homeGoals > awayGoals ? fixture.home : fixture.away);
          const playerResult = { pair: fixture, winner, homeGoals, awayGoals, penalties: homeGoals === awayGoals, home: homeClub.name, away: awayClub.name };
          const allResults = pending.roundData.resolved.concat([playerResult]);
          const finish = League.finishCupRound(phase, allResults);
          summary.eliminated = winner !== state.club.id;
          summary.advanced = winner === state.club.id;
        ...
        state.matchLog.unshift({ text: `${homeClub.name} ${homeGoals}x${awayGoals} ${awayClub.name} (você não foi relacionado)`, rating: null, week: state.week });
        if (state.matchLog.length > 40) state.matchLog.length = 40;
