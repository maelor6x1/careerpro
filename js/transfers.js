...
const Transfers = (() => {
  ...
  function maybeGenerateTransferOffer(state) {
    ...
    const chance = clamp01((interestScore - 40) / 220);
  ...
  function generateSponsorOffers(player) {
    if (player.popularity < 15) return [];
  ...
  function checkCallUp(player, state) {
    if (player.overall < 68 && player.caps === 0) return null;
    const chance = clamp01((player.overall - 62) / 90 + player.form / 400);
    if (Math.random() > chance) return null;
    const competition = pick(DB.NATIONAL_COMPETITIONS);
