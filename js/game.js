...
(function boot() {
  ...
  function handleTabClick(e) {
    ...
    if (e.target.closest('#btnRequestRaise')) {
      const res = Career.requestRaise();
      const out = document.getElementById('raiseResult');
      if (out) out.innerHTML = `<p class="${res.accepted ? 'success' : 'warn'} small">${res.message}${res.moraleDropped ? ' Sua moral caiu.' : ''}</p>`;
      return;
    }
    if (e.target.closest('#btnRequestRenewal')) {
      const res = Career.requestEarlyRenewal();
      const out = document.getElementById('renewalResult');
      if (out) out.innerHTML = `<p class="${res.accepted ? 'success' : 'warn'} small">${res.message}${res.moraleDropped ? ' Sua moral caiu.' : ''}</p>`;
      return;
    }
    if (e.target.closest('#btnRequestLeave')) {
      const res = Career.requestToLeave();
      if (res.accepted) { showContractDecision(); return; }
      const out = document.getElementById('leaveResult');
      if (out) out.innerHTML = `<p class="warn small">${res.message} Sua moral caiu.</p>`;
      return;
    }
  ...
  function startRound() {
    ...
    if (packet.nationalTournamentActive) { startTournamentRound(); return; }
  ...
  function startTournamentRound() {
    const roundInfo = Career.prepareNationalTournamentRound();
    if (roundInfo.tournamentOver) {
      UI.renderRoundSummary({ played: false, competition: 'Torneio de Seleções', eliminated: true, otherResults: [] }, () => processPopupQueue());
      return;
    }
    const first = Career.beginNationalTournamentMatch(roundInfo);
    ...
    const stageLabel = { group: 'Fase de grupos', quarter: 'Quartas de Final', semi: 'Semifinal', final: 'Final' }[roundInfo.stage] || roundInfo.stage;
    const matchInfo = { homeTeamObj, awayTeamObj, competition: `${m.competition} — ${stageLabel}`, isHome: m.isHome };
    UI.renderLiveMatch(matchInfo);
    wireLiveMatchSkip(matchInfo, true, true);
    wireLiveMatchSpeed();
    advanceLiveMatch(matchInfo, first, true, false, true);
  }
  ...
  function wireLiveMatchSkip(matchInfo, isNational, isTournament) {
    ...
    btn.addEventListener('click', () => {
      ...
      if (packet.matchOver) finishLiveMatch(matchInfo, isNational, isTournament);
      else advanceLiveMatch(matchInfo, packet, isNational, true, isTournament);
    });
  }

  function advanceLiveMatch(matchInfo, packet, isNational, skipDelay, isTournament) {
    if (packet.matchOver) { finishLiveMatch(matchInfo, isNational, isTournament); return; }
    ...
    if (ev.kind === 'decision' && !m.unavailable) {
      ...
      el.querySelectorAll('[data-opt]').forEach(btn => btn.addEventListener('click', () => {
        ...
        setTimeout(() => advanceLiveMatch(matchInfo, next, isNational, false, isTournament), Math.min(450, liveSpeedMs));
      }));
      return;
    }
    ...
    // Injury or red card just happened — show it immediately, then fast-
    // forward straight to full time instead of watching the rest tick by.
    const mAfter = Career.getState().currentMatch;
    if (mAfter && mAfter.unavailable && !matchInfo._incidentShown) {
      matchInfo._incidentShown = true;
      const incident = mAfter.ctx.injury
        ? { title: '🚑 Lesão!', text: `Você sofreu uma lesão: ${mAfter.ctx.injury.type}. Ficará ${mAfter.ctx.injury.weeksLeft} semana(s) fora dos gramados.` }
        : { title: '🟥 Expulso!', text: 'Você recebeu cartão vermelho e foi expulso de campo. O jogo segue sem você.' };
      UI.showIncidentCard(incident, () => {
        let p2 = next;
        while (!p2.matchOver) { const r2 = Career.stepMatch(); p2 = r2.next; }
        const finalMatch = Career.getState().currentMatch;
        if (finalMatch) UI.updateLiveScore(finalMatch.score, matchInfo.isHome);
        finishLiveMatch(matchInfo, isNational, isTournament);
      });
      return;
    }

    setTimeout(() => advanceLiveMatch(matchInfo, next, isNational, false, isTournament), skipDelay ? 0 : liveSpeedMs);
  }

  function finishLiveMatch(matchInfo, isNational, isTournament) {
    const summary = isTournament ? Career.concludeNationalTournamentMatch() : isNational ? Career.concludeNationalMatch() : Career.concludeRound();
    UI.renderRoundSummary(summary, () => {
      if (isTournament && Career.getState().nationalTournament) startTournamentRound();
      else processPopupQueue();
    });
  }
  ...
  function showContractDecision() {
    const state = Career.getState();
    const renewalOffer = state.player.contract ? Career.getRenewalOffer() : null;
