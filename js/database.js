...
const DB = (() => {
  ...
  // The football-logos repo (used below for the 7 European leagues) only
  // covers the top European leagues — it never had Brazilian or Argentine
  // clubs. For those, we fetch the real crest from Wikipedia's public
  // summary API (which returns each article's infobox image) once at boot
  // and cache it, since that endpoint returns JSON rather than a direct
  // image URL and can't be used as a plain <img src>.
  ...
  const crestCache = {}; // clubId -> resolved image URL, filled in by prefetchSouthAmericanCrests()
  let prefetchStarted = false;
  function prefetchSouthAmericanCrests(onComplete) {
  ...
  const NATION_REPUTATION = {
    Brasil: 88, Argentina: 87, França: 86, Inglaterra: 85, Espanha: 84, Portugal: 83,
    Alemanha: 83, Holanda: 81, Itália: 81, Bélgica: 80, Croácia: 78, Uruguai: 79,
    Colômbia: 77, México: 74, 'Estados Unidos': 73, Japão: 73, Marrocos: 76, Senegal: 74,
    Chile: 72, Peru: 68, Equador: 71, Paraguai: 66, Bolívia: 62, Venezuela: 65,
    Canadá: 68, Nigéria: 72, 'Coreia do Sul': 71,
  };
  ...
  const CONTINENTAL = {
    CONMEBOL: { top: 'Libertadores', second: 'Sul-Americana', leagues: ['BRA', 'ARG'] },
    UEFA: { top: 'Champions League', second: 'Europa League', leagues: ['ENG', 'ESP', 'ITA', 'GER', 'FRA', 'POR', 'NED'] },
  };
  ...
  return {
    ...
    getCrest, getFlagCrest, getNationalTeam, continentalFor, allNationTeamNationalities, getSponsorLogo, prefetchSouthAmericanCrests,
