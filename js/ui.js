...
const UI = (() => {
  ...
  function renderTab(state) {
    ...
    if (activeTab === 'competicoes') {
      el.innerHTML = tplCompeticoes(state);
      el.querySelectorAll('.comp-tab').forEach(btn => btn.addEventListener('click', () => {
        activeCompTab = btn.dataset.comptab;
        renderTab(state);
      }));
    }
  ...
  function tplCompeticoes(state) {
    const entries = [];
    if (state.competitions.estadual) entries.push(['estadual', state.competitions.estadual.name, state.competitions.estadual]);
    if (state.competitions.liga) entries.push(['liga', state.competitions.liga.name, state.competitions.liga]);
    entries.push(['copa', state.competitions.copa ? state.competitions.copa.name : (state.competitions._argCupName || 'Copa'), state.competitions.copa]);
    if (state.competitions.continental) entries.push(['continental', state.competitions.continental.name, state.competitions.continental]);

    if (!activeCompTab || !entries.find(e => e[0] === activeCompTab)) activeCompTab = entries[0][0];

    const tabsHtml = entries.map(([key, name]) => `<button class="comp-tab ${activeCompTab === key ? 'active' : ''}" data-comptab="${key}">${name}</button>`).join('');
    const [, , activePhase] = entries.find(e => e[0] === activeCompTab);
    const body = activePhase
      ? competitionBlock(state, activeCompTab, activePhase)
      : `<div class="card"><h3>${entries.find(e => e[0] === activeCompTab)[1]}</h3><p class="muted">O chaveamento será definido quando as duas zonas da fase de grupos terminarem.</p></div>`;

    return `<div class="comp-tabs">${tabsHtml}</div><div id="compTabBody">${body}</div>`;
  }
  ...
  function renderStandingsRows(standings, myClubId, leagueCode) {
    const cutoffs = leagueCode ? qualificationCutoffs(leagueCode) : null;
    return standings.map((s, i) => {
      const tierClass = cutoffs ? (i < cutoffs.top ? 'qualify-top' : i < cutoffs.top + cutoffs.second ? 'qualify-second' : '') : '';
      return `
  ...
  // How many table positions earn a continental berth, by literal row
  // position — this drives the color-coding, independent of the
  // reputation-based estimate used to seed clubs that don't have a real
  // simulated table (see Career.qualificationTierFor).
  function qualificationCutoffs(leagueCode) {
    if (!DB.continentalFor(leagueCode)) return null;
    return { top: 4, second: 2 };
  }
  ...
  function renderRoundSummary(summary, onClose) {
    const ov = ensureOverlay();
    const etPkBadge = (summary.wentToExtraTime || summary.wentToPenalties) ? `
      <div class="et-pk-badge">
        ${summary.wentToExtraTime ? '<span class="pill">⏱️ Prorrogação</span>' : ''}
        ${summary.wentToPenalties && summary.penaltyScore ? `<span class="pill ${summary.penaltyScore.weWin ? 'pill-gold' : ''}">🥅 Pênaltis: ${summary.penaltyScore.us} x ${summary.penaltyScore.them}</span>` : ''}
      </div>` : '';
