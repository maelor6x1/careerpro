...
const UI = (() => {
    ...
    <div class="hub">
      ...
      <div class="hub-body">
        <nav class="tabs">
          ...
          ${tabBtn('treino', '🏋️ Treino')}${tabBtn('inbox', '📨 Inbox' + (state.inbox.length ? ` <span class="badge">${state.inbox.length}</span>` : ''))}
  ...
  function renderTopbar(state) {
    ...
    return `<header class="topbar" style="--club-accent:${c1}">
      ...
      <div class="topbar-meta"><span>Temporada ${state.season}</span></div>
    </header>`;
  }
  ...
  function tplCompeticoes(state) {
    ...
    blocks.push(competitionBlock(state, 'liga', state.competitions.liga));
    blocks.push(competitionBlock(state, 'copa', state.competitions.copa));
    return blocks.join('');
  }
  ...
  function matchHeader(info) {
    const oppClub = info.opponent;
    return `<div class="match-header">
      <div class="match-team">${crestImg(info.club, 44)}<span>${info.club.name}</span></div>
      <div class="match-vs">${info.competition}<br><small>${info.isHome ? 'Casa' : 'Fora'} · Rodada ${info.roundNumber}/${info.totalRounds}</small></div>
      <div class="match-team">${crestImg(oppClub, 44)}<span>${oppClub.name}</span></div>
    </div>`;
  }

  function renderMatchDecision(info, beat, log) {
    const ov = ensureOverlay();
    ov.innerHTML = `<div class="modal card match-modal">
      ${matchHeader(info)}
      <div class="match-log">${(log || []).map(l => `<p class="event-line">${l}</p>`).join('')}</div>
      <div class="decision-box pulse">
        <p class="decision-prompt">${beat.prompt}</p>
        <div class="decision-options">
          ${beat.options.map(o => `<button class="btn-decision" data-opt="${o.key}">${o.label}</button>`).join('')}
        </div>
      </div>
    </div>`;
    return ov;
  ...
  function renderMatchResult(info, resultText, log, onContinue) {
    const ov = ensureOverlay();
    ov.innerHTML = `<div class="modal card match-modal">
      ${matchHeader(info)}
      <div class="match-log">${(log || []).map(l => `<p class="event-line">${l}</p>`).join('')}</div>
      <div class="result-banner">${resultText}</div>
      <button class="btn-primary" id="matchContinueBtn">Continuar</button>
    </div>`;
    ov.querySelector('#matchContinueBtn').addEventListener('click', onContinue);
    return ov;
  }
  ...
  return {
    ...
    renderMatchDecision, renderMatchResult, renderRoundSummary, closeOverlay,
