...
const UI = (() => {
  ...
  function renderContractDecision(state, renewalOffer, onDone) {
    ...
    <div class="screen reveal-screen fade-in">
      <div class="reveal-card card">
        <span class="eyebrow">Fim de contrato</span>
        <h2>${p.name}</h2>
        <p class="muted">Seu vínculo com o ${state.club.name} chegou ao fim. Hora de decidir o futuro da carreira.</p>
      </div>
      <div class="card">
    ...
    document.querySelectorAll('.league-pick').forEach(btn => btn.addEventListener('click', () => {
      const clubs = Career.getInterestedClubs(btn.dataset.league);
      const card = document.getElementById('newClubCard');
      if (!clubs.length) { card.querySelector('.league-grid').insertAdjacentHTML('afterend', '<p class="warn small">Nenhum clube dessa liga demonstrou interesse no momento.</p>'); return; }
      card.innerHTML = `<h3>Clubes interessados</h3><div class="proposal-grid">${clubs.map((c, i) => proposalCard(c, i)).join('')}</div>`;
      card.querySelectorAll('[data-choose]').forEach(b => b.addEventListener('click', () => {
        Career.signWithNewClub(clubs[parseInt(b.dataset.choose)]);
        onDone();
      }));
    }));
  }
  ...
  function renderStandingsRows(standings, myClubId) {
    return standings.map((s, i) => `
      <tr class="${s.clubId === myClubId ? 'current' : ''}">
  ...
  function competitionBlock(state, key, phase) {
    ...
    if (phase.type === 'liga') {
      ...
      return `<div class="card"><h3>${phase.name} ${activeBadge}</h3>${wrapStandingsTable(renderStandingsRows(standings, state.club.id))}</div>`;
    }
  ...
  function renderRoundSummary(summary, onClose) {
    ...
    ` : `<p class="muted">Sua equipe folgou nesta rodada.</p>`;
